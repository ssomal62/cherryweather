...
const ClubDetails = () => {
  ...
  const { loading: loadingClubMembershipData } = useMembershipData({
    state: currentClubMembershipInfoState,
    dynamicPath: `/${clubId}/memberships`,
  });

  const { loading: loadingMyMembership } = useMembershipData({
    state: currentMembershipState,
    dynamicPath: `/${clubId}/member`,
  });

  const { loading: loadingClubData } = useClubData({
    state: clubDetailState,
    dynamicPath: `/${clubId}`,
  });

  const { loading: loadingFeedData } = useFeedData({
    state: feedClubListState,
    dynamicPath: `/club/${clubId}`,
  });

  const loading =
    loadingMyMembership ||
    loadingClubData ||
    loadingClubMembershipData ||
    loadingFeedData;
  ...
  const clubProfile = (code) => {
    return `https://ffkv1pqc2354.edge.naverncp.com/p5Rq2SwoqV/club-profile/${
      code ? code : "default"
  ...
  return (
    <Layout
      useHeader={false}
      useFooter={false}
      containerMargin="0 0 100px 0"
      containerPadding="0 0 100px 0"
    >
      <ClubDetailsHeader
        clubDetail={clubDetail}
        isLogin={isLogin}
      ...
      <ClubDetail>
        <div style={styles.aspectRatio}>
          <Content>
            ...
            <EventSection clubDetail={clubDetail} />
            <ClubNotice
              clubDetail={clubDetail}
              hostName={hostName}
